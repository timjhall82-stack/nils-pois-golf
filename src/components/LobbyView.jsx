...
import { User, LogIn, AlertCircle, Contact, History, Info, ChevronDown, Map } from 'lucide-react'; // Added Map
...
const LobbyView = ({ 
    ...
    handleJoinGame, courseName, setCourseName, startSetup, error, 
    setShowPortal, setShowHistory, setShowCourseManager, // Added prop
    user, handleLogin, handleLogout, setShowInfo, savedPlayers, APP_VERSION, CUSTOM_LOGO_URL 
}) => {
    
    const handleProfileSelect = (e) => {
        const selectedId = e.target.value;
        const profile = savedPlayers.find(p => p.id === selectedId);
        if (profile) {
            setPlayerName(profile.name);
            setHandicapIndex(profile.handicap);
            setCurrentAvatar(profile.avatarUrl || '');
        }
    };

    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-transparent text-white space-y-6">
        ...
        {/* Action Buttons with 4 columns now */}
        <div className="w-full max-w-sm grid grid-cols-4 gap-2">
            ...
            <button onClick={() => setShowCourseManager(true)} className="bg-slate-900/60 hover:bg-slate-800/80 backdrop-blur-sm border border-white/5 p-3 rounded-xl flex flex-col items-center justify-center group transition-all shadow-lg"><div className="bg-orange-500/20 p-2 rounded-lg text-orange-400 mb-1 group-hover:bg-orange-500 group-hover:text-white transition-colors"><Map size={18} /></div><div className="font-bold text-[10px] text-slate-300">Courses</div></button>
        ...
        <div className="w-full max-w-sm bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl border border-white/10 space-y-4 shadow-2xl">
            <div className="flex justify-between items-center">
                <label className="text-[10px] font-bold text-slate-500 uppercase ml-1">Your Details</label>
                {savedPlayers && savedPlayers.length > 0 && (
                    <select className="bg-slate-800 text-[10px] text-blue-400 border border-slate-700 rounded px-2 py-1 outline-none max-w-[120px]" onChange={handleProfileSelect} value=""><option value="" disabled>Load Profile...</option>{savedPlayers.map(p => (<option key={p.id} value={p.id}>{p.name}</option>))}</select>
                )}
            </div>
            
            <div className="flex gap-2">
                <div className="relative flex-1">
                    <input type="text" className="w-full bg-slate-800/50 border border-slate-700 rounded-xl p-3 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white pl-10" value={playerName} onChange={(e) => setPlayerName(e.target.value)} placeholder="Name"/>
                    <div className="absolute left-3 top-3 text-slate-500">{currentAvatar ? <img src={currentAvatar} className="w-5 h-5 rounded-full" /> : <User size={18} />}</div>
                </div>
                <input type="number" className="w-20 bg-slate-800/50 border border-slate-700 rounded-xl p-3 text-sm focus:outline-none focus:border-blue-500 transition-colors text-white" value={handicapIndex} onChange={(e) => setHandicapIndex(e.target.value)} placeholder="HCP"/>
            </div>
