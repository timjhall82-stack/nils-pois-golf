...
const LobbyView = ({ 
    ...
    setShowPortal, setShowHistory, user, handleLogin, handleLogout, setShowInfo, savedPlayers, APP_VERSION, CUSTOM_LOGO_URL 
}) => {
    
    // Helper to auto-fill data when selecting a saved player
        ...
        <div className="w-full max-w-sm bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl border border-white/10 space-y-4 shadow-2xl">
            ...
            <div className="flex justify-between items-center">
                ...
                {savedPlayers && savedPlayers.length > 0 && (
                    <select 
                        className="bg-slate-800 text-[10px] text-blue-400 border border-slate-700 rounded px-2 py-1 outline-none max-w-[120px]" 
            ...
            <div className="flex gap-2">
                <div className="relative flex-1">
                    ...
                    <div className="absolute left-3 top-3 text-slate-500">
                        {currentAvatar ? <img src={currentAvatar} className="w-5 h-5 rounded-full" /> : <User size={18} />}
                    </div>
                </div>
